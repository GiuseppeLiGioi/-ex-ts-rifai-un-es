...
function isChef (obj : unknown) : obj is Chef {
    if(
        typeof obj === 'object' &&
         obj !== null &&
        'id' in obj &&
...
const getChefBirthday = async (id: number) : Promise <string | null> => {
    ...
    try{
        ...
        if(!responseRicetta.ok){
            throw new Error ("Non riesco a recuperare la ricetta con id" + id);
        }
    ...
    try{
        ...
        if (!isChef(infoChef)) {
      throw new Error("I dati ricevuti non corrispondono a uno chef valido.");
    }
